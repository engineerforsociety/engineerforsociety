-- ==========================================================
-- FINAL UNIFIED FEED VIEW & DATABASE FIXES
-- ==========================================================

-- 1. Ensure all social tables exist (just in case they were dropped or partially created)
...
-- 2. Add related_social_post_id to user_activities if not exists
...
-- 3. Update Activity Logging (Handle both Forum and Social)
...
BEGIN
    ...
    INSERT INTO public.user_activities (user_id, activity_type, activity_data, related_post_id, related_social_post_id)
    VALUES (
        target_author_id,
        target_activity_type,
        target_data,
        CASE WHEN TG_TABLE_NAME = 'forum_posts' THEN NEW.id ELSE NULL END,
        CASE WHEN TG_TABLE_NAME = 'social_posts' THEN NEW.id ELSE NULL END
    );
    RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 4. Re-create triggers (clean start)
...
-- 5. THE MASTER FEED VIEW (Security Invoker)
-- This view merges forum posts, social posts, and reposts.
-- It correctly handles likes/comments based on the post_type and item_type.
...
SELECT 
    ...
    -- Dynamic Stats
    CASE 
        WHEN aui.item_type = 'repost' THEN (SELECT COUNT(*) FROM public.repost_like_count_func(aui.repost_record_id)) -- Handled by subquery for precision
    ...
    -- Professional Auth-aware States
    COALESCE(
        CASE 
...
-- Helper functions for cleaner view if needed, but the above case logic is standard.
-- Note: Fixed the like_count subquery for reposts (use simple count from repost_reactions)
-- We will replace the like_count CASE for reposts with a direct count.

CREATE OR REPLACE VIEW public.feed_posts_view 
WITH (security_invoker = true) AS
WITH all_items AS (
    SELECT id, id as feed_item_id, title, content, tags, created_at, created_at as feed_created_at, COALESCE(view_count, 0) as view_count, COALESCE(is_pinned, false) as is_pinned, slug, author_id, 'forum'::text as post_type, 'post'::text as item_type, NULL::uuid as reposter_id, NULL::uuid as repost_record_id FROM public.forum_posts
    UNION ALL
    SELECT id, id as feed_item_id, NULL as title, content, NULL as tags, created_at, created_at as feed_created_at, 0 as view_count, false as is_pinned, slug, author_id, 'social'::text as post_type, 'post'::text as item_type, NULL::uuid as reposter_id, NULL::uuid as repost_record_id FROM public.social_posts
    UNION ALL
    SELECT COALESCE(fp.id, sp.id) as id, pr.id as feed_item_id, fp.title, COALESCE(fp.content, sp.content) as content, fp.tags, COALESCE(fp.created_at, sp.created_at) as created_at, pr.created_at as feed_created_at, 0 as view_count, false as is_pinned, COALESCE(fp.slug, sp.slug) as slug, COALESCE(fp.author_id, sp.author_id) as author_id, CASE WHEN fp.id IS NOT NULL THEN 'forum' ELSE 'social' END::text as post_type, 'repost'::text as item_type, pr.reposter_id, pr.id as repost_record_id FROM public.post_reposts pr LEFT JOIN public.forum_posts fp ON pr.original_post_id = fp.id LEFT JOIN public.social_posts sp ON pr.original_post_id = sp.id
)
SELECT 
    ai.*,
    p.full_name as author_name, p.avatar_url as author_avatar, p.job_title as author_title,
    pr.full_name as reposter_name, pr.avatar_url as reposter_avatar, pr.job_title as reposter_title,
    CASE WHEN ai.item_type = 'repost' THEN (SELECT COUNT(*) FROM public.repost_reactions WHERE repost_id = ai.repost_record_id) WHEN ai.post_type = 'forum' THEN (SELECT COUNT(*) FROM public.post_reactions WHERE post_id = ai.id AND reaction_type = 'like') ELSE (SELECT COUNT(*) FROM public.social_post_reactions WHERE post_id = ai.id) END as like_count,
    CASE WHEN ai.item_type = 'repost' THEN (SELECT COUNT(*) FROM public.repost_comments WHERE repost_id = ai.repost_record_id) WHEN ai.post_type = 'forum' THEN (SELECT COUNT(*) FROM public.forum_comments WHERE post_id = ai.id) ELSE (SELECT COUNT(*) FROM public.social_comments WHERE post_id = ai.id) END as comment_count,
    COALESCE(CASE WHEN ai.item_type = 'repost' THEN EXISTS(SELECT 1 FROM public.repost_reactions WHERE repost_id = ai.repost_record_id AND user_id = auth.uid()) WHEN ai.post_type = 'forum' THEN EXISTS(SELECT 1 FROM public.post_reactions WHERE post_id = ai.id AND user_id = auth.uid()) ELSE EXISTS(SELECT 1 FROM public.social_post_reactions WHERE post_id = ai.id AND user_id = auth.uid()) END, false) as is_liked,
    COALESCE(CASE WHEN ai.item_type = 'repost' THEN EXISTS(SELECT 1 FROM public.repost_saves WHERE repost_id = ai.repost_record_id AND user_id = auth.uid()) WHEN ai.post_type = 'forum' THEN EXISTS(SELECT 1 FROM public.post_saves WHERE post_id = ai.id AND user_id = auth.uid()) ELSE EXISTS(SELECT 1 FROM public.social_post_saves WHERE post_id = ai.id AND user_id = auth.uid()) END, false) as is_saved,
...
LEFT JOIN public.profiles pr ON ai.reposter_id = pr.id;
